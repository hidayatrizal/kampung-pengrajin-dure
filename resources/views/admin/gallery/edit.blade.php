...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-stone-100 p-6 md:p-10">
        <h2 class="font-serif text-2xl text-stone-900 mb-8">Edit Foto Galeri</h2>
        ...
        <form method="POST" action="{{ route('admin.gallery.update', $gallery) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category', $gallery->category) }}" required
                ...
                <div class="md:col-span-2">
                    <label class="text-[11px] uppercase tracking-[0.2em] text-stone-500 font-semibold block mb-2">Gambar</label>
                    <div class="relative">
                        @if($gallery->url)
                        ...
                        @endif
                        <input type="file" name="url" id="url" accept="image/jpeg,image/png,image/webp" class="hidden" onchange="previewImage(this, 'preview-gallery')">
                        <label for="url" id="preview-gallery" class="block border-2 border-dashed border-stone-200 rounded-xl p-6 text-center cursor-pointer hover:border-terracotta-400 hover:bg-terracotta-50/30 transition-all">
                            <svg class="w-8 h-8 text-stone-300 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                            <p class="text-sm text-stone-400">Klik untuk ganti gambar</p>
...
function previewImage(input, previewId) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            var currentImage = document.getElementById('current-image-gallery');
            ...
            preview.innerHTML = '<img src="' + e.target.result + '" class="w-full max-h-64 object-contain rounded-xl">' +
                ...
                '<button type="button" onclick="resetImage(\'url\', \'' + previewId + '\')" class="mt-2 text-xs text-red-500 hover:text-red-700">Hapus gambar</button>';
        };
...
function resetImage(inputId, previewId) {
    ...
    preview.innerHTML = '<svg class="w-8 h-8 text-stone-300 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>' +
        '<p class="text-sm text-stone-400">Klik untuk ganti gambar</p>' +
        '<p class="text-xs text-stone-300 mt-1">Kosongkan jika tidak ingin mengubah</p>';
}
</script>
